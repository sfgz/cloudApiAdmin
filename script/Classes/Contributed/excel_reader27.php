 ...
 *
 * Version 2.21 
 * dr: changes for PHP 7 on Feb. 2018. Changed Name from excel_reader2.php to excel_reader27.php
 * dr: boundsheets contains name and offset of Worksheets: boundsheets[1][name]
 * dr: sheets contains the cells. sheets[1] = array( numRows , numCols, cells => array( rowNr => array( fieldNr => fieldContent ) ) )
 *
...
class OLERead {
	var $data = '';
	function __construct(){	$this->startOLERead(); }
	function startOLERead(){	}
...
class Spreadsheet_Excel_Reader {
	...
	function __construct($filename){	$this->startSpreadsheet_Excel_Reader($filename); }
	function myHex($d) {
	...
	function hyperlink($row,$col,$sheet=0) {
		$link = !isset($this->sheets[$sheet]['cellsInfo'][$row][$col]['hyperlink']) ? '' : $this->sheets[$sheet]['cellsInfo'][$row][$col]['hyperlink'];
		if ($link) {
			return $link['link'];
		}
		return '';
	...
	function asArray($row_numbers=false,$col_letters=false,$sheet=0) {
		$outRow = array();
		$out = "<table>";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			...
			for($i=1;$i<=$this->colcount($sheet);$i++) {
				$out .= "\n\t\t<th>" . strtoupper($this->colindexes[$i]) . "</th>";
				$outRow[0][$i]= strtoupper($this->colindexes[$i]) ;
			}
			$out .= "</tr></thead>\n";
		}
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
					...
					$out .=  ($val!='') ? htmlentities($val) : ''; 
					$outRow[$row][$col]=($val!='') ? htmlentities($val) : ''; 
					$out .= "</td>";
			}
			$out .= "</tr>\n";
		}
	 ...
	 */
	function startSpreadsheet_Excel_Reader($file='',$store_extended_info=true,$outputEncoding='') {
